...
const inTheLibrary =
  window.location.pathname === "/library/titles" ||
  window.location.pathname === "/library" ||
  window.location.pathname === "/lib";
if (inTheLibrary) {
  $(function() {
    var filtersLast = "";
    ...
    // New library
    filtersLast = $(
      "#center-3 > div.bc-container > div.bc-row-responsive.adbl-library-refinement-section > div:nth-child(1) > span:nth-child(3)"
    );
    // Old Library
    if (filtersLast.length < 1) {
      filtersLast = $(
        "#center-3 > div > div > div > div.bc-col-responsive > div > form > span:nth-child(4)"
      );
      libraryStyle = "old";
    }
...
function audibleLibraryExtractor(oldLibraryData, libraryStyle) {
  $(
    DOMPurify.sanitize('<div id="audible-library-extractor"></div>', {
  ...
  new Vue({
    ...
    mixins: [timeStringToSeconds, secondsToTimeString],
    data: {
      partialScan: false,
    ...
    methods: {
      ...
      getInitialLibraryData1: function(pageNumbers, url) {
        ...
        if (vue.partialScan) {
          vue.progress.text =
            "Updating old books (" +
            vue.localStorageBooksLength +
            ") and adding new books...";
        }

        const requestUrls = _.map(pageNumbers, function(page) {
        ...
        Promise.all(
          options.request.map(function(url, index, array) {
            return axios
              ...
              .catch(function(e) {
                return e.response
                  ? options.step(e.response, index, array)
                  : null;
              });
          })
        )
      ...
      getlibraryPageData: function(response) {
        ...
        $(titleRows).each(function() {
          ...
          const partialScan_New =
            (vue.partialScan && !bookInMemory) || !vue.partialScan;
        ...
        if (urlSources.length > 0) {
          ...
          vue.ajaxios({
            ...
            step: function(response, index, array) {
              const book = _.find(vue.library.books, [
                "url",
                response.config.url
              ]);
      ...
      getStorePageData: function(response, book) {
        ...
        const jsonData = JSON.parse(
          html.find("#bottom-0 > script:first")[0].textContent
        );
      ...
      getCollections1: function() {
        ...
        if (queryURLS.length > 0) {
          vue.progress.text =
            "Fetching series order for more accurate sorting...";
          vue.progress.step = 0;
      ...
      */
      ...
      fixDates: function(el) {
        if (el) {
          ...
          const originalFormat =
            regionalDateFormats[domainExtension][1] ||
            regionalDateFormats[".com"][1];
      ...
      carouselDataFetch: function(parentBook, audible, key, carouselID) {
        const carousel = $(
          audible.querySelector("#adbl-web-carousel-c" + carouselID)
        );
        if (carousel.length > 0) {
          ...
          popover.each(function() {
            ...
            const image = flyout
              .prev(".responsive-product-square")
              .find('[id^="product-carousel-image"]');
            ...
            if (subHeading.length > 0)
              book.subHeading = subHeading.text().trim();
            listItems.length = 4;
      ...
      fetchISBNs: function(urlSources) {
        ...
        const booksAPIaddresses = _.filter(
          vue.library.books,
          "booksAPIaddress"
        );
        ...
        if (queryURLS.length > 0) {
          vue.progress.text =
            "Fetching international standard book numbers (ISBN)...";
          vue.progress.step = 0;
          vue.progress.maxLength = queryURLS.length;
            ...
            step: function(response, index, array) {
              ...
              if (
                ...
                const api_book = _.find(api_books, function(ab) {
                  const exists = {};
                  exists.author = _.find(ab.volumeInfo.authors, function(
                    author
                  ) {
                  ...
                  exists.title =
                    _.lowerCase(ab.volumeInfo.title).lastIndexOf(
                      lowercase_title
                    ) > -1;
