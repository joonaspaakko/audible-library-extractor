...
function getBooks( vue, response, hotpotato, completeStep ) {
  ...
  $(wishlistRows).each(function () {
    ...
    // ASIN
    if ( titleLink ) book.asin = titleLink.getAttribute('id').replace('title-', '');
    const bookInMemory = !hotpotato.config.partialScan ? undefined : _.find( hotpotato.wishlist, ['asin', book.asin] );
    
    if ( !bookInMemory ) {
      
      // TITLE
      if ( titleLink ) { book.title = titleLink.textContent.trimAll(); }
      
      // COVER
      const cover = _thisRow.querySelector('img[data-lazy]').getAttribute('src');
