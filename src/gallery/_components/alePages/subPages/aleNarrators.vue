<template>
  <div id="ale-narrators" class="box-layout-wrapper" v-if="$store.state.pageCollection && $store.state.pageCollection.length && listReady" :style="optionsOpenMargin" ref="wrapper">
    ...
    updateListRenderingOptions: function() {
      ...
      const list = {
        scope: [
          ...
          { active: true,  key: 'authors.name', tippy: 'Search narrators by authors' },
          { active: true,  key: 'publishers.name', tippy: 'Search narrators by publishers' },
          { active: true,  key: 'series.name', tippy: 'Search narrators by series' },
        ],
        filter: [
