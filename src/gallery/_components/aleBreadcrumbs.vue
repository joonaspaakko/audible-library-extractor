<template>
  <div id="ale-breadcrumbs" v-if="this.general.categories">
    ...
    <div v-if="crumbs.parent">
      <router-link
        class="crumb"
        :to="{ name: 'category', params: { parent: crumbs.parent.slug } }"
        >{{ crumbs.parent.name }}</router-link
      >
    </div>
    ...
    <div v-if="crumbs.child">
      <router-link
        ...
        :to="{
          name: 'category',
          params: { parent: crumbs.parent.slug, child: crumbs.child.slug }
        }"
        >{{ crumbs.child.name }}</router-link
      >
    </div>
    ...
    <div v-if="crumbs.asin">
      <router-link
        ...
        :to="{
          name: 'category',
          params: { parent: crumbs.parent.slug, child: crumbs.child.slug },
          query: { book: crumbs.asin.slug }
        }"
        >{{ crumbs.asin.name }}</router-link
      >
    </div>
  </div>
</template>
...
export default {
  ...
  created: function() {
    ...
    if (this.general.categories) {
      ...
      if (parentCat) {
        const parentCatObj = _.find(this.general.categories.parent, [
          "slug",
          parentCat
        ]);
        this.crumbs.parent = {
...
#ale-breadcrumbs {
  ...
  @include themify($themes) {
    ...
    box-shadow: 0 0 10px rgba(themed(outerColor), 0.5);
    border-top: 2px solid rgba(themed(frontColor), 0.1);
  }
  .divider {
    padding: 0 8px;
    @include themify($themes) {
