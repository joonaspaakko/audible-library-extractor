          ...
          <article class="media">
            ...
            <div class="media-content">
              <div class="content">
                ...
                <div>
                  <small>{{ props.row.releaseDate }}</small> •
                  <small>{{ props.row.length }}</small>
                </div>
...
export default {
  ...
  mixins: [makeFullUrl, makeCoverUrl],
      ...
      return _.map(vue.$store.state.booksArray, function(book) {
        ...
        _.each(vue.keys, function(key, i) {
          ...
          switch (key) {
            ...
            case "titleShort":
              const sample = book.sample
                ? '<a class="play-sample" href="' +
                  book.sample +
                  '" target="_blank">' +
                  ...
                  "</a>"
                : '<span class="play-sample-placeholder"></span>';

              newBook[key] =
                sample +
                '<a href="' +
                vue.makeFullUrl(book.url) +
                '" target="_blank">' +
                book.titleShort;
              "</a>";
              break;
...
#ale-spreadsheet {
  ...
  .pagination-next,
  .pagination-link {
    box-shadow: 1px 2px 6px rgba(#000, 0.2);
    @include themify($themes) {
      border-color: rgba(themed(frontColor), 0.2);
    }
  }
  ...
  .th-wrap {
    ...
    input {
      ...
      @include themify($themes) {
        background: darken(themed(backColor), 0.1);
      }
    }
  }
} // #ale-spreadsheet
