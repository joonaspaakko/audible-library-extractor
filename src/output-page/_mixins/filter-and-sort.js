...
export default {
	...
	methods: {
		...
		filterBooks: function( books ) {
			...
			const filterRules = _.filter( this.$store.state.sticky.listRenderingOpts.filter, ['active', true]);
		...
		sortBooks: function( books ) {
			...
			const sortByItem = _.find( this.$store.state.sticky.listRenderingOpts.sort, 'current');
			const sortOptions = {
				books: books,
			...
			const randomize = _.find( this.$store.state.sticky.listRenderingOpts.sortExtras, { key: 'randomize' });
